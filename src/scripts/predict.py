...
def generate_dummy_data():
    ...
    # ap_min_price = immo_apartment['Price'].min()
    # ap_max_price = immo_apartment['Price'].max()
    #
    # h_min_price = immo_house['Price'].min()
    # h_max_price = immo_house['Price'].max()
    ...
    for r in range(100):
        h_row = {
            ...
            'Bathroom Count': [random.randint(immo_house['Bathroom Count'].min(), immo_house['Bathroom Count'].max())],
            ...
            'Living Surface': [random.uniform(immo_house['Living Surface'].min(), immo_house['Living Surface'].max())],
        ...
        new_h_row['Room Count'] = new_h_row['Bedroom Count'] + new_h_row['Bathroom Count'] + new_h_row['Toilet Count']
    ...
    for i in range(100):
        h_prov_df.iloc[i, int(h_fake_df['Province'])] = 1
        ap_prov_df.iloc[i, int(ap_fake_df['Province'])] = 1
